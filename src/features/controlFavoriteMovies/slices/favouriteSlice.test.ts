...
describe('favourite slice test', () => {
  it('expect state add new hero', () => {
    ...
    expect(favoriteReducer(initialState, addFavourite(hero))).toEqual({
      heroes: [hero],
    });
  });
  ...
  it('expect state remove existed hero', () => {
    ...
    expect(favoriteReducer(initialState, removeFavourite(hero))).toEqual({
      heroes: [],
    });
  });
  ...
  it('expect state not remove not existed hero', () => {
    const initialState: FavouriteShema = { heroes: [hero] };
    expect(favoriteReducer(initialState, removeFavourite(second_hero))).toEqual(
      { heroes: [hero] }
    );
  });
  ...
  it('expect state should be clear', () => {
    ...
    expect(favoriteReducer(initialState, clearFavourite())).toEqual({
      heroes: [],
    });
  });
});
